<template>
  <div class="mobile full_height">
    ...
    <list class="cont full_width" :items="this.items" :pages_count="this.pages_count" :blog_num="this.blog_num" v-on:page_up="page_up" v-on:page_down="page_down">
    </list>
        ...
        .then(res => {
          ...
          if (Cookie.getCookie("blogToken")) {
            for (var i = 0; i < 2; i++) {
              ...
              if (
                this.items[i].likes_users.indexOf(
            ...
            // have not registered
            for (var i = 0; i < 2; i++) this.items[i].is_liked = false;
          }
        });
    }
  }
